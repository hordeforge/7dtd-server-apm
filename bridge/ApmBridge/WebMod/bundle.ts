...
interface PanelProps {
  ...
  useQuery: (key: string, fn: () => Promise<Any>, opts?: { refetchInterval?: number }) => {
    data?: Any;
    isError?: boolean;
...
const mib = (bytes: Any): number => num(bytes) / 1048576;
...
function grade(update: Any): Grade {
  ...
  if (avg === 0) return { tps, cls: "apm-warn", label: "NO DATA" };
  if (tps >= 19 && lateShare < 0.1) return { tps, cls: "apm-ok", label: "HEALTHY" };
  if (tps >= 10) return { tps, cls: "apm-warn", label: "DEGRADED" };
...
function rising(series: number[]): boolean {
...
function spark(React: Any, values: number[], color: string, w: number, h: number): Any {
...
function bar(React: Any, frac: number, cls: string): Any {
...
function ApmPanel({ React, HTTP, useQuery }: PanelProps): Any {
  ...
  const unwrapSnap = (o: Any): Any => {
    const s = o && o.data && typeof o.data === "object" ? o.data : null;
    if (s && s.data && typeof s.data === "object") return s.data;
    if (s && (s.schema || s.update || s.enabled)) return s;
    return o || {};
  };

  const hist = React.useRef({ last: null, tps: [] as number[], alloc: [] as number[], gm: [] as number[], gen2: [] as number[], heap: [] as number[] });
  const [frozen, setFrozen] = React.useState(false);
  const frozenSnap = React.useRef(null as Any);
  ...
  if (query.isError) {
    ...
    return h("div", { className: "seven-dtd-apm" },
      h("h2", null, "7DTD APM"),
      h("span", { className: "apm-pill apm-bad" }, "UNAVAILABLE"),
      h("p", null, msg));
  }
  ...
  const lu: Any = live.update || {};
  const lgc: Any = live.gc || {};
  if (!frozen && live.utc && live.utc !== hist.current.last) {
    const H = hist.current;
    H.last = live.utc;
    const avg = num(lu.serverTickIntervalAvgMs);
    const push = (arr: number[], v: number) => { arr.push(v); if (arr.length > HIST) arr.shift(); };
  ...
  const update: Any = data.update || {};
  const health: Any = data.health || {};
  const gc: Any = data.gc || {};
  const world: Any = data.world || {};
  const sections: SectionStat[] = data.sections || [];
  const transfers: TransferStat[] = data.mapTransfers || [];
  const spikes: SpikeRecord[] = data.spikes || [];
  const g = grade(update);

  const cell = (label: string, value: Any, cls: string | null) =>
    h("div", { className: "apm-cell" + (cls ? " " + cls : "") },
      h("span", { className: "apm-label" }, label),
      h("strong", null, value == null ? "—" : value));

  const trend = (label: string, series: number[], cur: string, color: string) =>
    h("div", { className: "apm-cell apm-trend" },
      h("span", { className: "apm-label" }, label),
      h("strong", null, cur),
      spark(React, series, color, 130, 30));

  const toggleFreeze = () => {
    if (!frozen) frozenSnap.current = live;
    setFrozen(!frozen);
  };
  const perfSnap: PerfState = unwrapSnap(perfQ.data);
  const perfEnabled = !!perfSnap.enabled;
  const perfAvailable = !!perfSnap.available;
  const togglePerf = async () => {
    if (perfBusy || !perfAvailable) return;
    setPerfBusy(true);
    try { await HTTP.post("/api/perf", { enabled: !perfEnabled }); }
    catch (e) { setPerfBusy(false); }
  };
  const copyJson = () => {
  ...
  const th = (label: string, key: string) =>
    h("th", { key: label, className: "apm-sortable", onClick: () => setSortKey(key) },
  ...
  return h("div", { className: "seven-dtd-apm" },
    h("div", { className: "apm-head" },
      ...
      h("span", { className: "apm-window" },
        "window " + fx(gc.windowSeconds, 0) + "s · " + num(update.windowUpdates) + " ticks" +
    ...
    h("div", { className: "apm-perf" },
      ...
      h("button", { className: "apm-btn", disabled: perfBusy || !perfAvailable, onClick: togglePerf },
        perfBusy ? "restarting server…" : (perfEnabled ? "Disable (restarts server)" : "Enable (restarts server)")),
      h("span", { className: "apm-window" }, "flips the config, restarts the server (~1-2 min)")),

    h("div", { className: "apm-tick-budget" },
      h("span", { className: "apm-label" }, "Tick vs 50 ms budget"),
      bar(React, num(update.serverTickIntervalAvgMs) / (TICK_BUDGET_MS * 2), g.cls === "apm-ok" ? "" : g.cls),
      h("span", { className: "apm-budget-val" }, fx(update.serverTickIntervalAvgMs, 1) + " ms")),

    h("div", { className: "apm-grid" },
      trend("TPS", H.tps, fx(g.tps, 1), "#57d977"),
      trend("Gross alloc MiB/s", H.alloc, fx(H.alloc[H.alloc.length - 1] || 0, 1), "#e6bd3a"),
      trend("gmUpdate avg ms", H.gm, fx(update.gmUpdateDurationAvgMs, 2), "#8ab4f8"),
      cell("Tick max", fx(update.serverTickIntervalMaxMs, 1) + " ms", null),
      cell("gmUpdate max", fx(update.gmUpdateDurationMaxMs, 1) + " ms", null),
      cell("Late ticks", num(update.lateTicks) + " (" + fx(update.tickStallMsTotal, 0) + " ms)", null),
      cell("Spikes", num(update.totalSpikes), null),
      cell("Players", num(world.players) + " / " + num(world.clients), null),
      cell("Entities", num(world.entities) + " (" + num(world.entityAlives) + " AI)", null),
      cell("GC gen0/s", fx(gc.gen0PerSecond, 1), null),
      cell("GC gen2/s", fx(gc.gen2PerSecond, 2), rising(H.gen2) ? "apm-warn" : null),
      cell("Heap", fx(mib(gc.heapBytes), 1) + " MiB", rising(H.heap) ? "apm-warn" : null),
      cell("Working set", fx(mib(world.workingSetBytes), 1) + " MiB", null),
      cell("Threads", num(world.threadCount), null),
      cell("Dropped exports", num(health.droppedExports), num(health.droppedExports) > 0 ? "apm-warn" : null)),
      ...
      h("tbody", null, shown.map((s) => {
        ...
        return h("tr", { key: s.name, className: rowCls },
          ...
          h("td", { className: "apm-budget-cell" },
            bar(React, frac, frac > 0.32 ? "apm-bad" : (frac > 0.1 ? "apm-warn" : "")),
            h("span", { className: "apm-budget-pct" }, fx(frac * 100, 1) + "%")));
      }))),

    spikes.length ? h("h3", null, "Recent spikes") : null,
    spikes.length ? h("table", { className: "apm-table" },
      h("thead", null, h("tr", null,
        ["When (UTC)", "gmUpdate ms", "Tick ms", "Players", "Entities"].map((x) => h("th", { key: x }, x)))),
      h("tbody", null, spikes.slice().reverse().slice(0, 12).map((s, i) =>
        h("tr", { key: i },
          h("td", null, (s.utc || "").replace("T", " ").replace(/\..*$/, "")),
          h("td", null, fx(s.gmUpdateDurationMs, 1)),
          h("td", null, fx(s.serverTickIntervalMs, 1)),
          h("td", null, num((s.world || {}).players)),
          h("td", null, num((s.world || {}).entities)))))) : null,

    h("h3", null, "Map and chunk transfers"),
    h("table", { className: "apm-table" },
      h("thead", null, h("tr", null,
        ["Package", "Count", "MiB", "Last bytes", "Max bytes"].map((x) => h("th", { key: x }, x)))),
      h("tbody", null, transfers.map((t) =>
        h("tr", { key: t.name },
          h("td", null, t.name),
