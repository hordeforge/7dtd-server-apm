(function () {
  ...
  function ApmPanel({ React, HTTP, useQuery }) {
    ...
    const trend = (label, series, cur, color) =>
    ...
    return h("div", { className: "seven-dtd-apm" },
      ...
      h("div", { className: "apm-grid" },
        ...
        cell("Tick max", fx(update.serverTickIntervalMaxMs, 1) + " ms"),
        cell("gmUpdate max", fx(update.gmUpdateDurationMaxMs, 1) + " ms"),
        cell("Late ticks", num(update.lateTicks) + " (" + fx(update.tickStallMsTotal, 0) + " ms)"),
        cell("Spikes", num(update.totalSpikes)),
        cell("Players", num(world.players) + " / " + num(world.clients)),
        cell("Entities", num(world.entities) + " (" + num(world.entityAlives) + " AI)"),
        cell("GC gen0/s", fx(gc.gen0PerSecond, 1)),
        ...
        cell("Working set", fx(mib(world.workingSetBytes), 1) + " MiB"),
        cell("Threads", num(world.threadCount)),
