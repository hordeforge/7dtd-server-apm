...
| Owns | Does not own |
|---|---|
| Process/CPU/sched/sync/runtime/memory/fs/block/net capture | Harmony performance “fixes” (use `7dtd-server-optimizer`) |
| Session store, audit, compare, budget, export, prune | Fake clients / protocol load (use `7dtd-loadgen`) |
| Optional `7dtd-server-apm-bridge.dll` instrumentation | Auto-editing EfficientServer or loadgen |
...
```bash
uv run 7dtd-server-apm capture --seconds 45 --only all
...
| `../7dtd-server-optimizer` | Reviewed optim patches; never auto-mutated by APM |
| `../7dtd-realearth` | Terrain product; optional world under test |
