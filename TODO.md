# 7dtd-server-apm implementation plan
...
may point at `7dtd-server-optimizer` without modifying it.
...
- [x] Move capture/finalization orchestration into the `7dtd-server-apm` Python CLI.
...
- R104: loadgen `--bot-mix` (weighted per-bot mode, deterministic by client id) -
  ...
  Wander:2/Demolition:2. Wired through run_loadgen.sh + `7dtd-server-apm scenario run
...
## Design review fix: --only token resolution unified (2026-08-23)
- Proven drift between capture planning and the audit over what an --only token means (8 disagreement cells): `--only all` flagged opt-in mono_alloc as "requested collector produced no usable evidence" in every default capture's manifest; `--only net` planned only io_net while audit/summary treated it as the whole io layer (false vfs/block warnings); the deliberate proc-with-threads ride-along was invisible to the audit. Fix: single predicate `models.collector_requested(name, layer, tokens, extra_aliases, optin)`; collector catalog (CaptureContext, CollectorSpec, SPECS, wanted, unknown_only_tokens) moved to a new leaf module `apm_suite/collectors.py` so capture planning and session.audit consume one catalog without an import cycle; per-spec layer aliases dropped (they duplicated LAYER_ALIASES), leaving only genuine extras (proc: threads; mono_alloc: alloc/allocsites). Behavior changes: `--only net` now plans vfs+block+io_net; `--only proc` also plans hw; audit no longer warns about unplanned opt-in collectors; failed proc counts as a requested gap under `--only threads`. Tests pin the plan/audit invariant across every token x collector pair plus the three behavior changes.
